...
btnC.addEventListener("click", () => { // Al botón se le adicióna el click y lo que sucede cuando éste evento sucede.
  ...
  document.getElementById("trad-text").innerHTML = window.cipher.encode(text, shifts); // Llama el resultado de la función para ponerlo en el sector correspondiente del HTML.
});

//Botón para la función descifrar.
const btnT = document.getElementById('btnT'); // Llama al botón del HTML.
btnT.addEventListener("click", () => { // Al botón se le adicióna el click y lo que sucede cuando éste evento sucede.
  let text = document.getElementById("source-text").value; // Se guarda el valor de "source-text" en la variable "text".
  let shifts = document.getElementById("shiftNumber").value; // Se guara el valor de "shiftNumber" en la variable "shifts".
  window.cipher.decode(text, shifts); // Se mandan los valores obtenidos a la función encode.
  document.getElementById("trad-text").innerHTML = window.cipher.decode(text, shifts); // Llama el resultado de la función para ponerlo en el sector correspondiente del HTML. 
});
}
